...
import { defineHandlerCallback as defineHandlerCallback$1, renderRouterToStream } from '@tanstack/react-router/ssr/server';
import { RouterProvider } from '@tanstack/react-router';

function StartServer(props) {
  return /* @__PURE__ */ jsx(RouterProvider, { router: props.router });
}
const defaultStreamHandler = defineHandlerCallback$1(
...
function defineHandlerCallback(handler2) {
  return handler2;
}
...
async function loadVirtualModule(id) {
  switch (id) {
    case VIRTUAL_MODULES.startManifest:
      return await import('../_/_tanstack-start-manifest_v-CSbuIo7m.mjs');
    case VIRTUAL_MODULES.serverFnManifest:
      return await import('../_/_tanstack-start-server-fn-manifest_v-CbeVCgVr.mjs');
...
function createStartHandler(cb) {
  ...
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import('../_/router-Dpgrn-8u.mjs').then((n) => n.r);
    }
    if (startEntry === null) {
...
async function handleServerRoutes({
  ...
  if (server2) {
    if (server2.handlers) {
      ...
      if (method) {
        const handler2 = handlers[method];
        if (handler2) {
          const mayDefer = !!foundRoute.options.component;
          if (typeof handler2 === "function") {
            middlewares.push(handlerToMiddleware(handler2, mayDefer));
          } else {
            const { middleware } = handler2;
            ...
            if (handler2.handler) {
              middlewares.push(handlerToMiddleware(handler2.handler, mayDefer));
            }
          }
        }
      }
...
function handlerToMiddleware(handler2, mayDefer = false) {
  ...
  return async ({ next: _next, ...rest }) => {
    const response = await handler2({ ...rest, next: throwIfMayNotDefer });
    if (!response) {
...
function splitSetCookieHeader(headerValue) {
  return headerValue.split(/,(?=\s*[^;=]+?=)/g).map((s2) => s2.trim()).filter(Boolean);
}
const handler = defineHandlerCallback(async (ctx) => {
  const res = await defaultStreamHandler(ctx);
  const h = res.headers;
  const direct = h.get("set-cookie");
...
const fetch$1 = createStartHandler(handler);
const server = { fetch: fetch$1 };
