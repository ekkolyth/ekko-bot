...
function ConnectDiscord() {
  const handleConnectDiscord = async () => {
    await authClient.signIn.social({
      provider: 'discord',
      callbackURL: '/auth/link',
    });
  };

  return (
    <div className="relative flex w-full h-screen justify-center items-center
      bg-[url('/discord.png')] bg-cover bg-center">

      {/* overlay */}
      <div className="absolute inset-0 bg-black/40" />

      <Card className="relative z-10 max-w-md p-8">
        <CardHeader className='p-0 text-2xl font-semibold'>Link your Discord account</CardHeader>
        <CardDescription className="mb-4 text-lg">
          To use Ekko Bot, you'll need to connect your existing Discord account.
        </CardDescription>
